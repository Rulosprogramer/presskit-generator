...
function ImageLibraryModal({ userId, isOpen, onClose, onSelectImage, imageType = 'gallery' }) {
  ...
  useEffect(() => {
    ...
    const loadLibrary = async () => {
      ...
      setError('');
      try {
        const images = await getImageLibrary({ userId });
        const sortedImages = [...images].sort((a, b) => {
          const aDate = new Date(a.uploadedAt?.toDate?.() || a.uploadedAt || 0).getTime();
          const bDate = new Date(b.uploadedAt?.toDate?.() || b.uploadedAt || 0).getTime();
          return bDate - aDate;
        });
        setLibrary(sortedImages);
      } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur">
      <div className="relative flex h-[min(92vh,56rem)] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-white/10 bg-zinc-950 p-6 shadow-xl">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-zinc-400">Aquí ves todas las imágenes que has subido en cualquier paso</p>
        </div>
              ...
              {library.map((image) => (
                ...
                >
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-linear-to-t from-black/60 to-transparent p-3 opacity-0 transition group-hover:opacity-100">
